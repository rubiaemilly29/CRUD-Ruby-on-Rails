class MarkCartAsAbandonedJob
  ...
  def perform
    ...
    Cart.where("updated_at < ? AND (abandoned = ? OR abandoned IS NULL)", 3.hours.ago, false).find_each do |cart|
      puts "Marcando carrinho #{cart.id} como abandonado"
      cart.mark_as_abandoned!
    end

    # Remove carrinhos abandonados há mais de 7 dias
    Cart.where("abandoned = ? AND updated_at < ?", true, 7.days.ago).find_each do |cart|
      puts "Removendo carrinho abandonado #{cart.id}"
